        ...
        <div className="py-12 space-y-4 ">
          {employerInfo.postedjobs.length ? (
            employerInfo.postedjobs.map((job, ind) => (
              <div
                className="bg-white p-5 rounded-md shadow-xl flex flex-col md:flex-row gap-6 md:gap-12 lg:gap-40 md:pb-20 relative"
                key={ind}
              >
                <div className="space-y-2">
                  <h1 className="text-lg font-semibold ">{job.jtitle}</h1>
                  <div className=" opacity-50">
                    {job.location} | Posted on : {job.date} |{" "}
                    {employerInfo.empname}
                  </div>
                </div>
